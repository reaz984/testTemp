...
if ($action == "load_group") {
    $sql = "select asset_group  from fam_acquisition_mst  where status_active=1 and is_deleted=0 and company_id =$data";
    echo "[" . substr(return_library_autocomplete($sql, "asset_group"), 0, -1) . "]";
    exit();
}

if ($action == "load_specification") {
    $sql = "select specification  from fam_acquisition_mst  where status_active=1 and is_deleted=0 and company_id =$data";
    ...
    <body>
        <div align="center" style="width:100%;" >
            <form name="SerialNoPopUpFrm_1"  id="SerialNoPopUpFrm_1" autocomplete="off">
                <table width="495" cellspacing="0" cellpadding="0" border="none" class="">
                    <tr>
                ...
                <table width="495" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center" id="tbl_serial_no">
                    <thead>
                    <th width="20">SL</th>
                    ...
                    <tbody>
                        ...
                        for ($c = 0; $c < count($txt_serial_no_hidden); $c++) {
                            ...
                            $asset_no_data_arr[$txt_serial_no_hidden_data[0]]['reg_no'] = $txt_serial_no_hidden_data[6];
                        }
                        if ($txt_qty != "") {
                            for ($i = 1; $i <= $txt_qty; $i++) {
                                ?>
                                <tr id="assetSlNo_<?php echo $i; ?>">
                                    ...
                                    <td><input type="text" name="txtSerialNo_<?php echo $i; ?>" id="txtSerialNo_<?php echo $i; ?>" class="text_boxes" style="width:120px" value="<?php echo $asset_no_data_arr[$i]['serial'] ?>"/> </td>
                                    <td>

                                        <?php if (str_replace("'", "", $cbo_aseet_type) == 10) { ?>
                                            <input type="text" name="txtRegNo_<?php echo $i; ?>" id="txtRegNo_<?php echo $i; ?>" class="text_boxes" style="width:100px" value="<?php echo $asset_no_data_arr[$i]['reg_no'] ?>"/>
                                        <?php } else { ?>
                                            <input type="text" name="txtRegNo_<?php echo $i; ?>" id="txtRegNo_<?php echo $i; ?>" class="text_boxes" style="width:100px" value="<?php echo $asset_no_data_arr[$i]['reg_no'] ?>"  readonly disabled="disabled"/>
                                        <?php } ?>
                                    ...
                                    <td><input type="text" name="txtWarranty_<?php echo $i; ?>" id="txtWarranty_<?php echo $i; ?>" class="datepicker" style="width:90px" value="<?php echo $asset_no_data_arr[$i]['warenty'] ?>" readonly/> </td>

                                    <td><input type="text" name="txtAssetNo_<?php echo $i; ?>" id="txtAssetNo_<?php echo $i; ?>" class="text_boxes" style="width:90px" readonly disabled="disabled" value="<?php echo $asset_no_data_arr[$i]['asset_no'] ?>"/></td> 

                                    <td><input type="button" name="decrease_1" id="decrease_1" value="-" class="formbutton" onClick="fn_deleteRow1(<?php echo $i; ?>)" style="width:50px"/>
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="6" align="center"><input type="button" name="btn" id="btn" value="Close" class="formbutton" onClick="serial_no_popup()" /></td>
                        </tr>
                    </tfoot>
                </table>    
                <td> 
    ...
    <script>
        ...
        function serial_no_popup() {
            ...
            for (var b = 1; b <= total_row; b++) {
                ...
                var txtRegNo = $("#txtRegNo_" + b).val();
                //alert(txthiddenSlUpdateId); 
                if (serial_no != '') {
                    serial_no += "*" + b + "_" + txtSerialNo + "_" + txtQty + "_" + txtWarranty + "_" + txtAssetNo + "_" + txthiddenSlUpdateId + "_" + txtRegNo;
                } else {
                    serial_no += b + "_" + txtSerialNo + "_" + txtQty + "_" + txtWarranty + "_" + txtAssetNo + "_" + txthiddenSlUpdateId + "_" + txtRegNo;
                }
            }
                ...
                <table width="980" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr>
                            <td>
                                <?php
                                echo create_drop_down("cbo_company_name", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name", 1, "-- Select Company --", $cbo_company_name, "load_drop_down( 'asset_acquisition_controller', this.value, 'load_drop_down_location', 'src_location_td');", "", "", "", "", "", "", "", "");
                                ?>
                            </td>
                            ...
                            <td>
                                <?php
                                echo create_drop_down("cbo_aseet_type", 110, $asset_type, "", 1, "--- Select ---", $selected, "load_drop_down( 'asset_acquisition_controller', this.value, 'load_drop_down_category', 'src_category_td' );", "", "", "", "", "", "", "", "");
                                ?>
                            </td>
...
if ($action == "show_searh_active_listview") {
    ...
    if ($db_type == 2) {//for oracal
        ...
        $sql = "SELECT  a.id, a.entry_no, c.asset_no, a.location, a.asset_type, a.asset_category, a.store, a.purchase_date, a.qty  FROM fam_acquisition_mst a, fam_acquisition_sl_dtls c  WHERE a.id=c.mst_id AND a.status_active=1 AND a.is_deleted=0 $category $aseet_type $location $company_id $asset_number $tran_date";
        //echo $sql;
    }
...
if ($action == "show_asset_active_listview") {
    ...
    //echo $data; die;	
    $sql = "SELECT  a.id, c.serial_no, c.asset_no, c.warranty_date FROM fam_acquisition_mst a, fam_acquisition_sl_dtls c  WHERE a.id = c.mst_id AND c.mst_id =$data AND c.status_active = 1 AND c.is_deleted = 0";

    echo create_list_view("list_view", "Serial No, Asset No, Warranty Date", "90,100", "400", "400", 0, $sql, "get_details_form_data", "id", "'populate_asset_details_form_data'", 1, "0,0,0", $arr, "serial_no,asset_no,warranty_date", "requires/asset_acquisition_controller", 'setFilterGrid("list_view",-1);', '0,0,3,0');
}
...
// Start: Insert Here----------------------------------------------------------
    if ($operation == 0) {
        ...
        $id = return_next_id("id", "fam_acquisition_mst", 1);
        if ($db_type == 2) {
        ...
        $field_array_serial = "id,asset_prifix,asset_type,asset_category,mst_id,serial_no,qty,warranty_date,asset_no,cost_per_unit,inserted_by,insert_date,reg_no";
        $asset_no_data_arr = array();
        for ($c = 0; $c < count($txt_serial_no_hidden); $c++) {

            $txt_serial_no_hidden_data = explode("_", $txt_serial_no_hidden[$c]);
            //print_r($txt_serial_no_hidden_data); die;

            $txt_warranty_date = $txt_serial_no_hidden_data[3];

            if ($db_type == 0)
                $warranty_date = change_date_format($txt_warranty_date, 'yyyy-mm-dd');
            if ($db_type == 2)
                $warranty_date = change_date_format($txt_warranty_date, 'DD-MMM-YYYY', '-', 1);
        ...
        for ($j = 1; $j <= str_replace("'", "", $txt_qty); $j++) {
            ...
            if ($data_array_serial != "")
                $data_array_serial.=",";

            $data_array_serial.="('" . $id_sl . "'," . $asset_prifix_id . "," . $cbo_aseet_type . "," . $cbo_category . ",'" . $id . "','" . $asset_no_data_arr[$j]['serial'] . "','1','" . $asset_no_data_arr[$j]['warenty'] . "','" . $asset_no . "'," . $txt_purchase_cost . ",'" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "','" . $asset_no_data_arr[$j]['reg_no'] . "')";
        ...
        $rID = sql_insert("fam_acquisition_mst", $field_array, $data_array, 0);
        $rID1 = 1;
        $rID2 = 1;
...
// Start : Update Here----------------------------------------------------------
        ...
        $field_array = "company_id*location*specification*asset_type*asset_category*asset_group*store*qty*recive_mode*serial_no*brand*origin*purchase_date*cost_per_unit*purc_currency*engine_no*accumu_dep*deprec_rate*depreciation_method*dia_width*gauge*extra_cylinder*no_of_feeder*prod_capacity*capacity_uom*sequence_no*supplier*supplier_id*asset_uom*salvage_value*updated_by*update_date";

        $data_array = "" . $cbo_company_name . "*" . $cbo_location . "*" . $txt_specification . "*" . $cbo_aseet_type . "*" . $cbo_category . "*" . $txt_asset_group . "*" . $cbo_store . "*" . $txt_qty . "*" . $cbo_rec_mode . "*" . $txt_serial_no . "*" . $txt_brand . "*" . $cbo_origin . "*" . $txt_purchase_date . "*" . $txt_purchase_cost . "*" . $cbo_purc_currency . "*" . $tex_engine_no . "*" . $txt_accumulated_dep . "*" . $txt_depreciation_rate . "*" . $cbo_depreciation_method . "*" . $txt_dia_width . "*" . $txt_gauge . "*" . $txt_extra_cylinder . "*" . $txt_no_of_feeder . "*" . $txt_prod_capacity . "*" . $cbo_capacity_uom . "*" . $txt_sequence_no . "*" . $txt_supplier . "*" . $hidden_supplier_id . "*" . $cbo_asset_uom . "*" . $txt_salvage_value . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
        //Start : Update data Set Qty PopUp  --------------txt_set_qty_hidden-------------------------
        $txt_set_qty_hidden_row = explode("*", str_replace("'", "", $txt_set_qty_hidden));

        $id_set = return_next_id("id", "fam_acquisition_set_dtls", 1);
        $field_array_acquisition_set = "id,mst_id,part_name,qty_set,serial_no,inserted_by,insert_date";
        $field_array_acquisition_set_update = "part_name*qty_set*serial_no*updated_by*update_date";
        $data_array_acquisition_set = '';

        for ($a = 0; $a < count($txt_set_qty_hidden_row); $a++) {
            $acquisition_set_popup_data = explode("_", $txt_set_qty_hidden_row[$a]);
            if ($acquisition_set_popup_data[3] != "") {
                $update_set_arr[] = $acquisition_set_popup_data[3];
                $data_array_acquisition_set_arr[$acquisition_set_popup_data[3]] = explode("*", ("'" . $acquisition_set_popup_data[0] . "'*'" . $acquisition_set_popup_data[1] . "'*'" . $acquisition_set_popup_data[2] . "'*'" . $_SESSION['logic_erp']['user_id'] . "'*'" . $pc_date_time . "'"));
            } else {
                if ($data_array_acquisition_set != "")
                    $data_array_acquisition_set.=",";
                $data_array_acquisition_set.="('" . $id_set . "'," . $update_id . ",'" . $acquisition_set_popup_data[0] . "','" . $acquisition_set_popup_data[1] . "','" . $acquisition_set_popup_data[2] . "','" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "')";
                $id_set = $id_set + 1;
            }
        }

        if (str_replace("'", "", $hidden_set_deleted_id) != "") {
            //$hidden_set_deleted_id = explode(",", $hidden_set_deleted_id);
            $field_array_update_set_dtls = "status_active*is_deleted*updated_by*update_date";
            $data_array_update_set_dtls = "'0'*'1'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
            $set_delete = sql_multirow_update("fam_acquisition_set_dtls", $field_array_update_set_dtls, $data_array_update_set_dtls, "id", str_replace("'", "", $hidden_set_deleted_id), '');
        }

        //Start : Update data Serial No PopUp  ----------------------txt_serial_no_hidden-----------------
        $txt_serial_no_hidden_row = explode("*", str_replace("'", "", $txt_serial_no_hidden));

        $id_sl = return_next_id("id", "fam_acquisition_sl_dtls", 1);
        $field_array_serial = "id,asset_prifix,asset_type,asset_category,mst_id,serial_no,qty,warranty_date,asset_no,cost_per_unit,inserted_by,insert_date,reg_no";
        $field_array_acq_serial_no_update = "serial_no*qty*warranty_date*cost_per_unit*updated_by*update_date*reg_no";


        $result_asset_prifix_id = sql_select("select max(asset_prifix) as prifix  from fam_acquisition_sl_dtls  where asset_type=$cbo_aseet_type and asset_category=$cbo_category ");
        $asset_prifix = str_pad(str_replace("'", "", $cbo_company_name), 2, "0", STR_PAD_LEFT) . "" . str_pad(str_replace("'", "", $cbo_aseet_type), 2, "0", STR_PAD_LEFT) . "" . str_pad(str_replace("'", "", $cbo_category), 3, "0", STR_PAD_LEFT);
        $asset_prifix_id = $result_asset_prifix_id[0][csf('prifix')] + 1;
        ...
        $data_array_serial = '';
        //$j=1;
        //for ($e = 0; $e < count($txt_serial_no_hidden_row); $e++) 
        for ($e = 0; $e < str_replace("'", "", $txt_qty); $e++) {
            $txt_serial_no_hidden_row_data = explode("_", $txt_serial_no_hidden_row[$e]);

            //print_r($txt_serial_no_hidden_row); die;

            $update_warrantyDate = $txt_serial_no_hidden_row_data[3];
            if ($db_type == 0)
                $warranty_date = change_date_format($update_warrantyDate, 'DD-MMM-YYYY');
            if ($db_type == 2)
                $warranty_date = change_date_format($update_warrantyDate, 'DD-MMM-YYYY', '-', 1);

            if ($txt_serial_no_hidden_row_data[5] != "") {
                $update_SerialNo_arr[] = $txt_serial_no_hidden_row_data[5];

                $data_array_serial_no_array[$txt_serial_no_hidden_row_data[5]] = explode("*", ("'" . $txt_serial_no_hidden_row_data[1] . "'*'" . $txt_serial_no_hidden_row_data[2] . "'*'" . $warranty_date . "'*" . $txt_purchase_cost . "*'" . $_SESSION['logic_erp']['user_id'] . "'*'" . $pc_date_time . "'*'" . $txt_serial_no_hidden_row_data[6] . "'"));

                $data_array_land_position_array[$txt_serial_no_hidden_row_data[5]] = explode("*", ("" . $txt_purchase_cost . "*'" . $_SESSION['logic_erp']['user_id'] . "'*'" . $pc_date_time . "'"));

                //$data_array_land_position_update = "" . $txt_purchase_cost . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
            } else {
                //echo "10**".$data_array_serial; die;
                ...
                $data_array_serial.="('" . $id_sl . "'," . $asset_prifix_id . "," . $cbo_aseet_type . "," . $cbo_category . "," . $update_id . ",'" . $txt_serial_no_hidden_row_data[1] . "','1','" . $warranty_date . "','" . $asset_no . "'," . $txt_purchase_cost . ",'" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "','" . $txt_serial_no_hidden_row_data[6] . "')";
                //echo "10**".$data_array_serial; die;
        ...
        //echo "10**insert into fam_acquisition_sl_dtls($field_array_serial) values".$data_array_serial;die;
        //echo $hidden_sl_deleted_id; die;
        if (str_replace("'", "", $hidden_sl_deleted_id) != "") {
            //$hidden_sl_deleted_id = explode(",", $hidden_sl_deleted_id);
            $field_array_update_sl_dtls = "status_active*is_deleted*updated_by*update_date";
            $data_array_update_sl_dtls = "'2'*'1'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";

            $sl_delete = sql_multirow_update("fam_acquisition_sl_dtls", $field_array_update_sl_dtls, $data_array_update_sl_dtls, "id", str_replace("'", "", $hidden_sl_deleted_id), '');
        }
        //End : Update data Serial No PopUp  ----------------------txt_serial_no_hidden-----------------
        //echo $field_array; die;
        $rID1 = sql_update("fam_acquisition_mst", $field_array, $data_array, "id", "" . $update_id . "", 0);
        $rID2 = 1;
        $rID3 = 1;
        $rID4 = 1;
        //$rID5 = 1;
        $rID25 = 1;
        if (count($data_array_acquisition_set_arr) > 0) {
            $rID2 = execute_query(bulk_update_sql_statement("fam_acquisition_set_dtls", "id", $field_array_acquisition_set_update, $data_array_acquisition_set_arr, $update_set_arr, 0), 1);
        }

        if ($data_array_acquisition_set != "") {
            $rID3 = sql_insert("fam_acquisition_set_dtls", $field_array_acquisition_set, $data_array_acquisition_set, 1);
        }
        //echo $data_array_land_position_update; die;
        //Update Serial No============================
        if (count($data_array_serial_no_array) >= 0) {
            $rID4 = execute_query(bulk_update_sql_statement("fam_acquisition_sl_dtls", "id", $field_array_acq_serial_no_update, $data_array_serial_no_array, $update_SerialNo_arr, 0), 1);
        }

        if (!empty($data_array_serial)) {

            $rID5 = sql_insert("fam_acquisition_sl_dtls", $field_array_serial, $data_array_serial, 1);
        }

        //For Land Position page Purchase Cost filed update.
        $field_array_land_position_update = "purchase_cost*updated_by*update_date";
        if (str_replace("'", "", $cbo_aseet_type) == 1) {
            $rID25 = execute_query(bulk_update_sql_statement("fam_land_position_mst", "asset_id", $field_array_land_position_update, $data_array_land_position_array, $update_SerialNo_arr, 0), 1);
            //echo bulk_update_sql_statement("fam_land_position_mst", "asset_id", $field_array_land_position_update, $data_array_land_position_array, $update_SerialNo_arr, 0)."*****"; die;
        }

        $txt_entry_no = str_replace("'", "", $txt_entry_no);
        $update_id = str_replace("'", "", $update_id);

        // echo "10**".$rID1."**".$rID2."**". $rID3."**".$rID4."**".$rID5."**".$rID25;  die;
...
// Start : Delete Here----------------------------------------------------------
        ...
        $rID2 = sql_delete("fam_acquisition_mst", $field_array, $data_array, "id", "" . $update_id . "", 1);
...
if ($action == "populate_asset_details_form_data") {
    ...
    foreach ($data_array as $row) {
        ...
        echo "load_drop_down('requires/asset_acquisition_controller','" . $row[csf("asset_type")] . "','load_drop_down_category','category_td' );\n";
        ...
        if ($row[csf("asset_type")] == 1 || $row[csf("asset_type")] == 2) {
            ...
            echo "document.getElementById('cbo_asset_uom').value 		= '" . $row[csf("asset_uom")] . "';\n";
            echo "document.getElementById('txt_qty').value 				= '" . $row[csf("qty")] . "';\n";
            echo "document.getElementById('hidden_txt_qty').value 				= '" . $row[csf("qty")] . "';\n";
        }
        ...
        echo "load_drop_down('requires/asset_acquisition_controller','" . $row[csf("company_id")] . "','load_drop_down_store','store_td' );\n";
        ...
        echo "document.getElementById('tex_engine_no').value 	= '" . $row[csf("engine_no")] . "';\n";
        ...
        //=======================Set=======================
        if ($row[csf("asset_uom")] == 58) {
            $data_array_qty = sql_select("select id,part_name,qty_set,serial_no from fam_acquisition_set_dtls where mst_id='$data' and status_active=1 and is_deleted=0");
            //print_r($data_array_qty);
            $set_break_down = "";
            foreach ($data_array_qty as $val) {
                if ($set_break_down != "")
                    $set_break_down.="*";
                $set_break_down.=$val[csf("part_name")] . "_" . $val[csf("qty_set")] . "_" . $val[csf("serial_no")] . "_" . $val[csf("id")];
            }

            echo "document.getElementById('txt_set_qty_hidden').value = '" . $set_break_down . "';\n";
        }
        //===========================Serial No===============================
        if ($row[csf("qty")] != "") {
            $data_array_sl = sql_select("select id,serial_no,qty,warranty_date,asset_no,reg_no from fam_acquisition_sl_dtls where mst_id = '$data'  and status_active=1 and is_deleted=0");
            //print_r($data_array_sl);
            $sl_break_down = "";
            $sl = 1;
            foreach ($data_array_sl as $val) {
                if ($sl_break_down != "")
                    $sl_break_down.="*";

                $sl_break_down.=$sl . "_" . $val[csf("serial_no")] . "_" . $val[csf("qty")] . "_" . $val[csf("warranty_date")] . "_" . $val[csf("asset_no")] . "_" . $val[csf("id")] . "_" . $val[csf("reg_no")];
                $sl++;
            }
            echo "document.getElementById('txt_serial_no_hidden').value = '" . $sl_break_down . "';\n";
        }
        //===============================================
