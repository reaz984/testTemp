...
if ($action == "show_asset_no_searh_listview") {
    ...
    if ($db_type == 0) {  //for MySql
        $sql = "SELECT  a.company_id, a.cost_per_unit, c.id, c.serial_no, c.asset_no FROM fam_acquisition_mst a, fam_acquisition_sl_dtls c  WHERE a.id=c.mst_id AND c.asset_type=2 AND  a.status_active=1 AND a.is_deleted=0 AND c.status_active=1 AND c.is_deleted=0 $asset_number $company_name $location";
    }
    ...
    echo create_list_view("list_view", "Asset No,Serial No,File Number,Deed Number", "112,112,112,112", "535", "330", 0, $sql, "js_asset_value", "asset_no,id,company_id,cost_per_unit", "", 1, "0,0,id,id", $arr, "asset_no,serial_no,id,id", "requires/building_position_controller", 'setFilterGrid("list_view",-1);', '0,0,0,0');
	exit;
}
...
// Start : Update Here----------------------------------------------------------
        ...
        $field_array = "asset_id*asset_no*company_id*land_location*address*mouja_name*file_number*amt_of_land*deed_number*east_owner*west_owner*south_owner*north_owner*deed_value*purchase_cost*pre_market_value*booking_date*booking_amount*selling_date*reg_office*court_fee*orig_deed_recpt_no*registry_property*copy_doc_rev_date*original_deed_rev_date*orig_deed_poss_coll_date*land_possession*possession_under*tax_paid_up_to*tax_payment_date*tax_maturity_date*havingCase*mortgage_given*mortgage_no*mortgage_date*mortgage_to*branch_name*which_company*updated_by*update_date*is_mutation*lease_deed*attorneys_permit_doc*sales_permit_number*sales_permit_date*rajuk_mutation*charter_inherit*soil_test_result*design_approved*developer_name*developer_address*price_per_sq_feet*total_square_feet*total_floor*car_parking_qty*generator_available*lift_available*lift_capacity*entry_form_name";
		...
		$rID=1; $rID11=1; $rID12=1; $rID13=1; $rID14=1; $rID15=1; $rID16=1;
...
if ($action == "populate_land_details_form_data") {
    ...
    foreach ($data_array as $row) {
		...
		//lease_deed, attorneys_permit_doc, sales_permit_number, sales_permit_date, rajuk_mutation, charter_inherit, soil_test_result, design_approved, developer_name , developer_address, price_per_sq_feet, total_square_feet, total_floor, car_parking_qty, generator_available, lift_available, lift_capacity
		echo "document.getElementById('cbo_lease_deed').value 			= '" . $row[csf("lease_deed")] . "';\n";
		echo "document.getElementById('cbo_attorneys_permit_doc').value = '" . $row[csf("attorneys_permit_doc")] . "';\n";
		echo "document.getElementById('txt_sales_permit_number').value 	= '" . $row[csf("sales_permit_number")] . "';\n";
		echo "document.getElementById('txt_sales_permit_date').value 	= '" . change_date_format($row[csf("sales_permit_date")], "dd-mm-yyyy", "-") . "';\n";
		echo "document.getElementById('cbo_rajuk_mutation').value 		= '" . $row[csf("rajuk_mutation")] . "';\n";
		echo "document.getElementById('cbo_charter_inherit').value 		= '" . $row[csf("charter_inherit")] . "';\n";
		echo "document.getElementById('txt_soil_test_result').value 	= '" . $row[csf("soil_test_result")] . "';\n";
		echo "document.getElementById('cbo_is_design_approved').value 	= '" . $row[csf("design_approved")] . "';\n";
		echo "document.getElementById('txt_developer_name').value 		= '" . $row[csf("developer_name")] . "';\n";
		echo "document.getElementById('txt_developer_address').value 	= '" . $row[csf("developer_address")] . "';\n";
		echo "document.getElementById('txt_price_per_sq_feet').value 	= '" . $row[csf("price_per_sq_feet")] . "';\n";
		echo "document.getElementById('txt_total_square_feet').value 	= '" . $row[csf("total_square_feet")] . "';\n";
		echo "document.getElementById('txt_total_floor').value 			= '" . $row[csf("total_floor")] . "';\n";
		echo "document.getElementById('txt_car_parking_qty').value 		= '" . $row[csf("car_parking_qty")] . "';\n";
		echo "document.getElementById('cbo_generator_available').value 	= '" . $row[csf("generator_available")] . "';\n";
		echo "document.getElementById('txt_lift_available').value 		= '" . $row[csf("lift_available")] . "';\n";
		echo "document.getElementById('txt_lift_capacity').value 		= '" . $row[csf("lift_capacity")] . "';\n";
