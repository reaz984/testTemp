...
if ($action == "show_file_number_search_listview") 
{
	...
	if ($db_type == 0) {		//for MySql
		$sql = "SELECT id,asset_no,company_id,file_number FROM fam_land_position_mst WHERE  havingcase=1 AND entry_form_name = 2 AND  status_active=1 AND is_deleted=0 $file_number $asset_number $company_name";
	}
	if($db_type == 2 || $db_type == 1) // for Oracale and MsSql
	{
		$sql = "SELECT id,asset_no,company_id,file_number FROM fam_land_position_mst WHERE  havingcase=1 AND entry_form_name = 2  AND status_active=1 AND is_deleted=0 $file_number $asset_number $company_name";
	}
    echo create_list_view("list_view", "File Number, Asset No, Company", "100,100", "400", "300", 0, $sql, "js_file_no", "file_number,asset_no,company_id,id", "", 1, "0,0,0",$arr, "file_number,asset_no,id", "requires/civil_case_for_building_controller", 'setFilterGrid("list_view",-1);', '0,0,0');
